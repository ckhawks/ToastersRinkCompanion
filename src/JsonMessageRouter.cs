            ...
            (senderClientId, reader) =>
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        Plugin.AddLocalChatMessage($"<size=14><color=red>There was no handler for a message received from the server; you might be on an outdated version of Toaster's Rink Companion! Type <b>/outdated</b> for help.</color></size>");
                    }
                }
                catch (Exception e)
                {
